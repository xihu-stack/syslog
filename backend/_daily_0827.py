...
R["一1_对账"] = {"错人": wrong, "悬空": dangle, "分不一致": mismatch, "severity错": sev_bad, "dedup重复": dupk}

# 一2: 抽5条当日告警对数
samples = []
for a in db.execute("""SELECT id, employee_id, scenario, risk_score, summary, window_start FROM alerts
    WHERE window_start>='2026-08-27' AND scenario IN ('mass_exfil','data_exfiltration') ORDER BY id DESC LIMIT 5""").fetchall():
    ...
    if m:
        ...
        samples.append((a["id"], a["employee_id"][:8], f"{m.group(1)}/{m.group(2)}MB", f"实{cnt}/{sz/1048576:.1f}MB", "OK" if ok else "DIFF"))
    else:
        samples.append((a["id"], a["employee_id"][:8], "非数字模板", "-", "-"))
R["一2_抽5条"] = samples

# 一3: 榜单复算
sys.path.insert(0, "/app")
import dicts
for emp in ("houshunan", "杨颖"):
    ...
    pk = db.execute("SELECT MAX(risk_score) FROM verdicts WHERE employee_id=?", (emp,)).fetchone()[0]
    R.setdefault("一3_复算", {})[emp] = {"cur": cur, "peak": pk}
R["一3_效率"] = "见API"

# ===== 二、漏报 =====
# 二1: 高风险域名无verdict
HR = ("filehelper", "zhipin", "liepin", "51job", "pan.baidu", "outlook.live", "mail.163", "mail.qq")
miss1 = []
for pat in HR:
    for r in db.execute(f"""SELECT employee_id, COUNT(*) c FROM events WHERE category='WEB'
        ...
        if hv == 0:
            ...
            miss1.append((r["employee_id"][:12], pat[:15], r["c"], f"7天{d7}次"))
R["二1_高风险无verdict"] = miss1[:8] or "无"
...
R["二3_深夜零研判"] = miss3[:8] or "无"

# 二5: deepaudit
try:
    from deepaudit import run_deep_audit
    R["二5_deepaudit"] = run_deep_audit()
except Exception as e:
    R["二5_deepaudit"] = f"失败:{e}"
...
for v in db.execute("SELECT id, employee_id, event_hashes FROM verdicts WHERE event_hashes IS NOT NULL ORDER BY id DESC LIMIT 3").fetchall():
    ...
    if "→NETWORK\n" in txt or "→NETWORK(网" in txt: issues.append("裸NETWORK")
...
agtm = json.loads(db.execute("SELECT value FROM settings WHERE key='ipg_agt_map'").fetchone()[0]) if db.execute("SELECT value FROM settings WHERE key='ipg_agt_map'").fetchone() else {}
R["五3_活跃IPG"] = [(r["employee_id"], agtm.get(r["employee_id"][4:]) or "未匹配") for r in new_ipg[:6]]
