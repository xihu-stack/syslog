...
if mismatch:
    for a in db.execute("SELECT id, employee_id, risk_score, verdict_id, summary FROM alerts WHERE verdict_id IS NOT NULL").fetchall():
        v = db.execute("SELECT employee_id, risk_score FROM verdicts WHERE id=?", (a["verdict_id"],)).fetchone()
        if v and v["employee_id"]==a["employee_id"] and not PREFIX.search(a["summary"] or "") and a["risk_score"]!=v["risk_score"]:
            R["一1_明细"] = f"[{a[0]}] {a[1][:10]} alert={a[2]} v={v[1]}"

samples = []
for a in db.execute("""SELECT id, employee_id, scenario, risk_score, summary, window_start FROM alerts
    WHERE window_start>=date('now','+8 hours') AND scenario IN ('mass_exfil','data_exfiltration') ORDER BY id DESC LIMIT 5""").fetchall():
    ...
    if m:
        ...
        samples.append((a["id"], a["employee_id"][:8], f"{m.group(1)}/{m.group(2)}MB", f"实{cnt}/{sz/1048576:.0f}", "OK" if ok else "DIFF"))
    else:
        cnt = q1("SELECT COUNT(*) FROM events WHERE employee_id=? AND occurred_at>=? AND occurred_at<? AND action IN ('SEND','UPLOAD')", a["employee_id"], d0, d0+" 23:59")
