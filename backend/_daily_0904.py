...
import dicts
R["0_LLM"] = {"enabled": dicts.get_setting("llm_enabled", "?")}
...
for a in db.execute("SELECT id, employee_id, risk_score, verdict_id, summary FROM alerts WHERE verdict_id IS NOT NULL").fetchall():
    ...
    if not PREFIX.search(a["summary"] or "") and a["risk_score"] != v["risk_score"]:
        mismatch += 1
        mismatch_detail.append(f"[{a[0]}]{a[1][:8]} a={a[2]} v={v[1]}")
...
R["一1"] = {"错人": wrong, "悬空": dangle, "分不一致": mismatch, "sev错": sev_bad, "dedup": dupk}
if mismatch_detail: R["一1_明细"] = mismatch_detail

# 一2: 用2天口径(与mass_exfil 48h scan一致)
samples = []
for a in db.execute("""SELECT id, employee_id, scenario, risk_score, summary, window_start FROM alerts
    WHERE window_start>=date('now','+8 hours','-1 day') AND scenario IN ('mass_exfil','data_exfiltration') ORDER BY id DESC LIMIT 5""").fetchall():
    d0 = str(a["window_start"])[:10]
    m = re.search(r"累计外发(\d+)次、共([\d.]+)MB", a["summary"] or "")
    if m:
        cnt = q1("SELECT COUNT(*) FROM events WHERE employee_id=? AND occurred_at>=datetime(?,'-1 day') AND occurred_at<? AND action IN ('SEND','UPLOAD')", a["employee_id"], d0, d0+" 23:59")
        sz = q1("SELECT COALESCE(SUM(size_bytes),0) FROM events WHERE employee_id=? AND occurred_at>=datetime(?,'-1 day') AND occurred_at<? AND action IN ('SEND','UPLOAD')", a["employee_id"], d0, d0+" 23:59")
        diff_pct = abs(int(m.group(1)) - cnt) / max(cnt, 1) * 100
        samples.append((a["id"], a["employee_id"][:8], f"{m.group(1)}/{m.group(2)}", f"2d实{cnt}/{sz/1048576:.0f}", "OK" if diff_pct < 30 else f"差{diff_pct:.0f}%"))
    else:
        ...
        samples.append((a["id"], a["employee_id"][:8], "AI", f"实{cnt}SEND", "OK"))
R["一2"] = samples
...
for emp in ("houshunan", "熊磊"):
    ...
    R.setdefault("一3", {})[emp] = {"cur": cur, "peak": mem[0] if mem else None, "trend": mem[2] if mem else None}
...
for v in db.execute("""SELECT id, employee_id, intent, risk_score, event_hashes FROM verdicts
    ...
    anch.append((v["id"], v["employee_id"][:8], f"v={v['risk_score']}", f"锚={sc}", "OK" if sc is None or abs(v["risk_score"]-sc)<=10 else f"偏{v['risk_score']-sc}"))
R["四2"] = anch
...
new_ipg = db.execute("""SELECT employee_id FROM events WHERE employee_id LIKE 'IPG:%'
