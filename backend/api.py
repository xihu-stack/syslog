...
def ask(body: dict = Body(...)):
    ...
    # 当前时间必须注入(2026-08-24用户实测: 问"今天礼拜几"AI答周三,实际周一——
    # LLM不知道现在时间,没这行就会靠训练记忆瞎猜)
    _nw = bj_now()
    _wd = ("一", "二", "三", "四", "五", "六", "日")[_nw.weekday()]
    sys_p = (f"当前时间: {_nw.strftime('%Y-%m-%d %H:%M')} 星期{_wd}(北京时间)。『今天/昨天/本周』等时间词一律以此为准,严禁按你训练记忆推算星期。\n\n"
             "你是『员工行为分析系统』的智能助手,服务对象是系统管理员(安全运营人员)。你可以调用查询工具获取真实数据。\n\n"
