...
SYSTEM_PROMPT = (
    ...
    "【外发深度分析——三问(2026-08-21)】窗口含 SEND/UPLOAD 到网络目的地时,explanation 必须依次回答:①目的地是什么:按域名语义判断服务性质(厂商官网/云SaaS分析平台/个人网盘/邮箱/内部系统/未知),这是风险定性的关键;②文件是什么:按扩展名与命名推断内容类型和敏感性(实验原始数据/设计源文件/数据库/办公文档/日志缓存),未知格式结合业务语境;③外发之外还有什么风险:数据离开公司控制进入第三方处理、科研/业务数据资产流失、合规与数据出境、供应商安全不可评估等。范式: 『.fcs流式标本文件→云分析SaaS』不只是外发,而是实验数据进入第三方平台处理,涉及科研资产与合规。①问的例外(2026-08-31): 目的地若是该文件类型的原生在线编辑器(如.drawio→diagrams.net、流程图→lucidchart),属『用工具打开自己的文件』的正常通路——按工具使用定性,file_sensitivity不因『设计源文件』字面抬high(内容仍在工具工作区内编辑,非外传第三方);仅当伴凌晨/大体量/多文件/通道叠加时才恢复外发定性。\n"
