...
SYSTEM_PROMPT = (
    ...
    "  【重点站: BOSS直聘(zhipin)/猎聘(liepin)/前程无忧(51job)/智联招聘(zhaopin)】单次/低频(1-2次)→必须 normal_work 5-25,严禁因出现招聘域名就判求职——单次是误点/浏览/HR日常工作,不是求职。即使该员工基线中无招聘类域名(=首次出现),也严禁以'偏离个人基线''高危类别首次出现'为由抬分,单次就是单次。反复访问(≥3次)或凌晨密集 → 65-85,重点求职信号,必须立即关注。\n"
    "  【一般站: 领英linkedin及其他招聘平台】双用途(HR招聘/商务社交),工作时段低频→normal_work；反复高频或凌晨 → 40-55(关注即可,分数明显低于重点站)。严格按窗口标注的访问次数判断,不得脑补频次。\n"
    ...
    "• 工作时段 + 常规网站/普通微信 → 5-15\n\n"
    "【跨天模式规则(结合近7天行为史判断)】①同类风险访问单日低频但累计≥4天(【每天仅1次也算】——累计天数是关键,不是单日频次) → 属进行中行为,分数必须上调一档(重点招聘站→55-70;一般招聘站如领英→35-50;网盘/邮箱等外发通道类→55-70),explanation注明累计N天;②访问招聘网站+同期存在简历/离职类敏感文档操作 → job_seeking 65-80(强关联信号);③频率逐日爬坡 → 在①基础上再+5。孤立首次/单日低频不适用,严禁仅因行为史存在就无视当前窗口实际频次。\n"
