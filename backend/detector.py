...
SYSTEM_PROMPT = (
    ...
    "• VPN/翻墙(privado/clash/shadowsocks等) → 75-85（翻墙违规+绕监控,严重）\n"
    "• 代码仓库(github/gitlab) → 40-55（代码外发嫌疑,看频次/时段）\n"
    "• 微信文件助手(传文件) → 70-80（外发嫌疑）\n"
    "• 招聘网站:单次/低频访问(窗口内1-2次) → 15-25（可能误点/正常浏览,不算求职）；反复访问(≥3次)或凌晨 → 50-70（求职意图）。按窗口里标注的访问次数×N 判断\n"
    "• 远程控制 + 凌晨 → 55-65；工作时段 → 30-40（降权）\n"
    "• AI助手(chatgpt/deepseek/豆包/kimi/copilot等) → 工作时段低频(1-3次) 5-15(正常使用,不算风险)；反复高频访问(≥10次)或凌晨 → 40-55(数据外发嫌疑:可能往AI塞公司数据)\n"
    "• 凌晨 + 仅常规网站(无外发通道) → 25-35\n"
    "• 工作时段 + 常规网站/普通微信 → 5-15\n\n"
    "【intent 判定】个人邮箱/网盘(禁止) → policy_violation；VPN/翻墙 → policy_violation；"
    "微信文件助手 → data_exfiltration；代码仓库 → baseline_deviation；"
    "招聘网站反复访问 → job_seeking（单次/低频 → normal_work）；远程控制 → baseline_deviation；"
    "AI助手反复高频访问 → data_exfiltration（低频 → normal_work）；普通微信/正常办公 → normal_work。\n"
