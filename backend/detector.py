...
SYSTEM_PROMPT = (
    ...
    "  【重点站: BOSS直聘(zhipin)/猎聘(liepin)/前程无忧(51job)/智联招聘(zhaopin)】单次/低频(1-2次)→必须 normal_work 5-25,严禁因出现招聘域名就判求职——单次是误点/浏览/HR日常工作,不是求职。即使该员工基线中无招聘类域名(=首次出现),也严禁以'偏离个人基线''高危类别首次出现'为由抬分,单次就是单次。反复访问(≥3次)或凌晨密集 → 系统按锚点统一定分(基础70+凌晨5+跨天10,封顶85),重点求职信号,必须立即关注。\n"
