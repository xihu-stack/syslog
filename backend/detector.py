...
def anchor_score(intent, window, day_total=None) -> int | None:
    ...
    # 写集须剔除外发/入站类动作(2026-08-31审计): WRITE_ACTIONS含SEND/UPLOAD,
    # 旧写集被SEND自身命中自己→任何外发窗口必得预谋+10(纯发无写高分案例实测)
    _lw = WRITE_ACTIONS - {"SEND", "UPLOAD", "PRINT", "BURN", "DOWNLOAD"}
    _wn = {_base(e.target_value) for e in window
           if e.category == "DOC" and e.action in _lw
    ...
    if _send_dests:
        ...
        _all_auto = all(any(k in str(e.target_value or "").lower() for k in _AUTO_NAME)
                        for e in _send_evs)
        # 伪装链(2026-09-07用户逆向思考): 压缩包外发不吃默认名降档——
        # 『新建压缩包.zip』字面无语义,但打包本身是刻意动作且内容对审计不可见,
        # 与随手截图/空壳工作簿不同性质
        _ARCH_EXT = (".zip", ".rar", ".7z", ".tar", ".gz")
        _arch = any(str(e.target_value or "").lower().endswith(_ARCH_EXT)
                    for e in _send_evs)
        if _all_auto and not _rare and not _arch:
            _floor = min(_floor, 75)
        if _arch:
            _floor += 5  # 压缩打包外发: 内容不可见的刻意动作
        # 外发前≤30分钟有RENAME(改名遮蔽原文件名再外发)→+5;不要求改名前后与
        # 外发同名(那是上方has_write同名链的职责),两条链互补
        if any(e.category == "DOC" and e.action == "RENAME" and e.occurred_at
               and s.occurred_at
               and 0 <= (s.occurred_at - e.occurred_at).total_seconds() <= 1800
               for e in window for s in _send_evs):
            _floor += 5
...
# ---- 准则条款表(2026-09-07条款化) ----
# SYSTEM_PROMPT 从散文补丁堆改为编号条款: 每条一个语义完整的判定口径块。
# id=条款号(供ruleaudit活性统计/判例蒸馏按条款引用/增删可追溯), since=口径
# 首次进入时间, src=出处。拼接字节级与条款化前完全一致(vLLM prefix caching
# 与判例匹配依赖字节稳定,test_facts.py锁sha256)。维护规则: 新增准则=新条款;
# 改口径=改对应条款文本并更新since; 禁止在条款表外拼接散文本。
_C = lambda cid, name, since, src, text: {"id": cid, "name": name, "since": since, "src": src, "text": text}
...
CLAUSES = [
    ...
    _C("C04", "公司业务画像", "v0", "facts.py单源(2026-09-07)", facts.BUSINESS_PROFILE_FACTS),
    _C("C05", "邮箱网盘禁令", "v0", "初始版", (
        "个人邮箱、网盘/云盘 在公司【禁止使用】→ 任何访问即违规(policy_violation),不管时段。\n"
    )),
    _C("C06", "公司域名事实", "2026-09-07", "facts.py单源", facts.COMPANY_DOMAIN_FACTS),
    _C("C07", "微信文件助手通道", "v0", "初始版", (
        "微信文件助手(filehelper/文件传输助手)=传文件外发通道 → 访问即外发嫌疑(data_exfiltration)。\n"
    )),
    _C("C08", "数据外发判定边界", "v0", "初始版", (
    ...
    _C("C09", "SendPhotoes通道噪音", "2026-08-20", "通道噪声审计", (
    ...
    _C("C10", "远程控制降权", "v0", "初始版", (
        "远程控制(todesk等)=工具使用,降权(凌晨/密集才告警)。\n"
    )),
    _C("C11", "删除行为判定", "v0", "初始版", (
    ...
    _C("C17", "锚点·招聘三规合一", "2026-09-01", "三规合一", (
        "• 招聘网站(2026-09-01三规合一——旧版'含重点站即75+强制intent'/'跨天60-70'/'仅反复才算'三条并存互相矛盾,字段跟一条、文字跟另一条,产出'说明写正常办公,字段却job_seeking@75-80'的自相矛盾判定):\n"
    ...
    _C("C18", "领英北森HR口径", "2026-08-28", "italent误标审计", (
    ...
    _C("C19", "求职渠道不限字典", "2026-09-04", "漏报复盘", (
        "  【求职渠道不限于字典站(2026-09-04漏报复盘)】各公司招聘页常挂在主域子域(talent./career./jobs./employer.开头,如talent.deepseek.com、careercenter.asco.org),ATS平台(mokahr/理聘lipind/丁香人才jobmd/jobsDB)也承载各公司职位页——这些域名出现在窗口=求职信号,按一般站口径定档;标题《面试官答复/筛选简历/人才库》=招聘方工作,不算求职。技术社区的面经/求职攻略文章(juejin等)反复浏览可作求职佐证写进说明,单次不定job_seeking。\n"
    )),
    _C("C20", "离职推断铁律", "v0", "初始版", (
        "  【离职推断准确性铁律】判job_seeking前必须先回答:这是本人找工作,还是在做招聘工作/岗位事务?①输入含豁免标注的岗位(如HR),其招聘站操作是岗位工作——explanation严禁写『进行中求职行为/跳槽』,应写『招聘岗位工作(已豁免)』,分数按低档;②窗口主信号是外发/删除/AI使用时,夹杂的单次招聘访问不得改变intent,按主信号定性,招聘只在explanation附带一句;③《搜索人才/人才库/候选人》类页面标题是招聘方视角,不是求职;④个人事务搜索(离职证明/社保停缴/公积金提取/工资水平查询)单次=normal_work;但同窗或近日反复出现,或与招聘访问/简历操作并存→属离职前兆信号,可判job_seeking 65-75,explanation写明组合依据(如'招聘访问+社保停缴搜索'),勿当孤立生活事务放过;⑤跨天累计只能引用行为史『招聘求职』那一行,严禁拿AI/邮箱等其他场景的跨天给求职升档。\n"
    )),
    _C("C21", "锚点·远程控制", "v0", "初始版", (
        "• 远程控制 + 凌晨 → 55-65；工作时段 → 30-40（降权）\n"
    )),
    _C("C22", "锚点·AI助手", "v0", "初始版", (
    ...
    _C("C23", "锚点·凌晨常规", "v0", "初始版", (
        "• 凌晨 + 仅常规网站(无外发通道) → 25-35\n"
    )),
    _C("C24", "锚点·工作时段常规", "v0", "初始版", (
        "• 工作时段 + 常规网站/普通微信 → 5-15\n\n"
    )),
    _C("C25", "跨天模式规则", "v0", "初始版", (
        "【跨天模式规则(结合近7天行为史判断)】①同类风险访问单日低频但累计≥4天(【每天仅1次也算】——累计天数是关键,不是单日频次) → 属进行中行为,招聘类【当前窗口含招聘访问→判 job_seeking 且分数≥50进告警】(重点站分档同上【评分锚点】招聘条:单次主信号60/反复80/高频85,跨天+5封顶95;一般站(脉脉/看准/卓聘等,非领英——领英已确认正常)→50-60,经常访问必须注意但分数不用太高),explanation注明累计N天;②访问招聘网站+同期存在简历/离职类敏感文档操作 → job_seeking 65-80(强关联信号);③频率逐日爬坡 → 在①基础上再+5。孤立首次/单日低频不适用,严禁仅因行为史存在就无视当前窗口实际频次。【①的场景作用域铁律】行为史按风险类逐行给出,每行的跨天标记只对该类自身生效:判定某场景时只看同类那一行的累计天数(判求职只看招聘求职行、判邮箱违规只看个人邮箱行);其他场景(AI助手/远程控制等)的跨天标记仅作背景,严禁据此给当前场景升档或写进说明(2026-08-28审计: 多个邮箱/AI窗口引用了他场景的跨天行)。\n"
    )),
    _C("C26", "风险域名不进基线", "v0", "初始版", (
        "【风险域名不属于基线】招聘/网盘/邮箱/文件助手等风险类域名【永不计入个人基线或全局常见】——即使该员工天天访问、即使在其常用域名中,也绝不构成正常理由;天天访问恰是进行中风险信号(跨天模式)。\n"
    )),
    _C("C27", "基线偏离边界", "v0", "初始版", (
    ...
    _C("C30", "外发四问", "2026-08-21", "三问→09-07增④", (
        "【外发深度分析——四问(2026-08-21三问,2026-09-07增④伪装检查)】窗口含 SEND/UPLOAD 到网络目的地时,explanation 必须依次回答:①目的地是什么:按域名语义判断服务性质(厂商官网/云SaaS分析平台/个人网盘/邮箱/内部系统/未知),这是风险定性的关键;②文件是什么:按扩展名与命名推断内容类型和敏感性(实验原始数据/设计源文件/数据库/办公文档/日志缓存),未知格式结合业务语境;并对照该员工画像基线——平时不碰此类文件的人突然外发该类(如实验岗员工发『个人照片.zip』),文件名与工作内容错位本身就是伪装信号,须写进说明;③外发之外还有什么风险:数据离开公司控制进入第三方处理、科研/业务数据资产流失、合规与数据出境、供应商安全不可评估等;④外发前有什么前置动作:同窗口内若存在『RENAME改名→紧接外发』『外发压缩包(.zip/.rar/.7z等)』『批量打开与外发文件名明显不符的敏感文件』——这是改名/压缩遮蔽真实文件名的伪装外发特征,explanation必须点明『疑似伪装外发』并引用改名前后文件名或压缩包名;压缩包外发的file_sensitivity至少mid(打包动作本身即刻意,内容对审计不可见,不能因名字无害判none)。范式: 『.fcs流式标本文件→云分析SaaS』不只是外发,而是实验数据进入第三方平台处理,涉及科研资产与合规。①问的例外(2026-08-31): 目的地若是该文件类型的原生在线编辑器(如.drawio→diagrams.net、流程图→lucidchart),属『用工具打开自己的文件』的正常通路——按工具使用定性,file_sensitivity不因『设计源文件』字面抬high(内容仍在工具工作区内编辑,非外传第三方);仅当伴凌晨/大体量/多文件/通道叠加时才恢复外发定性。\n"
    )),
    _C("C31", "在职利益冲突", "2026-09-07", "新增", (
        "【在职利益冲突留痕(2026-09-07)】搜索词/页面标题/外发对象中出现私活接单、飞单、在外设立公司、私下承接业务等在职牟利证据(如自营公司名+对外报价/开票流程/接单平台操作)→intent 用 baseline_deviation,explanation 以『疑似在职牟利:』开头并引用证据原文;仅弱联想(兼职广告闪现/猎头沟通等)不得使用该表述,如实按对应场景判定。\n"
    )),
    _C("C32", "语义判断总则", "v0", "初始版", (
    ...
    _C("C33", "intent判定枚举", "v0", "初始版", (
        "【intent 判定】个人邮箱/网盘(禁止) → policy_violation；VPN/翻墙 → normal_work(业务规则:翻墙非违规)；"
        "微信文件助手 → data_exfiltration；代码仓库 → baseline_deviation；"
        "招聘网站反复高频(≥3)/凌晨 → job_seeking；单次但为窗口主信号 → job_seeking(60关注留痕档)；单次低频夹杂 → 按窗口主信号(多为normal_work)；远程控制 → baseline_deviation；"
    ...
    _C("C36", "explanation五W", "2026-08-26", "示例防抄袭", (
    ...
    _C("C39", "总次数口径心跳", "2026-08-26", "心跳计数审计", (
    ...
    _C("C41", "说明时间铁律", "2026-08-26", "时间错位审计", (
...
SYSTEM_PROMPT = "".join(c["text"] for c in CLAUSES)
